...
#define Leftshift(input, n) (((input) >> (28 - n)) | ((input << n) & 0xfffffff))
...
ull F_fun(ull input, ull Keyroll)
{
    ...
    ull outputtemp = 0;
    ull output = 0;
    input = newSub_E(input);     //扩展置换E
    temp = input ^ Keyroll;      //异或运算
    outputtemp = Sub_Sbox(temp); // S盒代替
    return outputtemp;
}
void IPFinal(ull leftt, ull right)
{
    ull work;
...
ull DES(ull input)
{
    ...
    ull R0 = (input)&0xffffffff;         //低32位
    ull L0 = (input >> 32) & 0xffffffff; //高32位
    ull Rroll = R0;
    ull Lroll = L0;
    ull Rtemp = 0;
    ull Ltemp = 0;
...
int main()
{
    int n;
    ull input, key;
    scanf("%d%llx%llx", &n, &input, &key);
    Sub_key(key);
    ull temp, output;
    ull L = (input) & (0xffffffff00000000L);
    hex_print(L);
    ull R = (input)&0xffffffffL;
    hex_print(R);
    IPFinal(L, R);
    hex_print(L);
    hex_print(R);
    ...
    output = Sub_IPinverse(temp); // IP逆置换
    hex_print(output);
    return 0;
}
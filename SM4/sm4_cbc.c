...
void CBC_Print(int mode, int flag, int last)
{
    int i;
    if (mode == 1)
    {
...
void CBC_Cipher(int mode,ull input[],ull Etemp[],int flag)
{
    if (mode == 1)
    { //加密时需要先进行异或运算
        input[0] = input[0] ^ Etemp[0];
        input[1] = input[1] ^ Etemp[1];
    }
    ...
    {
        Output[flag] = Output[flag] ^ Etemp[0];
        Output[flag + 1] = Output[flag + 1] ^ Etemp[1];
        Etemp[0] = input[0]; //解密时本轮的输入作为下一轮的初始向量
        Etemp[1] = input[1];
    }
}
int main()
{
    ...
    KeyExtend(Key);
    ull Etemp[5] = {};
    Etemp[0] = StrtoHex(IV, 2, 16); //初始向量IV
    Etemp[1] = StrtoHex(IV, 18, 16);
    ull input[2] = {};
    ull temp = 0, count = 0;
    ull Memory = 0, move = 7;
    while (scanf("%x", &temp) != EOF)
    {
        ...
        {    
            count = 0;
            CBC_Cipher(mode,input,Etemp,flag);
            flag += 2;
        }
    }
    ull last = 0;

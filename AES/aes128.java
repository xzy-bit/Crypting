...
public class aes128 {
    ...
    static long[] word = new long[45];  //字的总数为Nb*(Nr+1)
    static long[] dword = new long[45];
    static int[][] W = new int[4][45]; //将轮密钥存储起来
    static int[][] DW = new int[4][45];
    ...
    public static void KeyExpansion(String Key, long w[], int Nk) {
        String[] strarray = new String[20];
        for (int j = 0; j < 34; j += 2) {
    ...
    public static void AESCipher(int[][] state) {
    ...
    public static void AESInverseCipher(int[][] state) {
        GetDW();
        MemoryDWord(dword, 44);
        /*System.out.print("Start:");
        StateHexPrint(state);
        System.out.print(" ");*/
        AddRoundKeyInverse(state, 10);
        for (int i = 9; i > 0; i--) {
    ...
    public static void AES(int[][] state, int mode, int n) {
        if (mode == 1) {
            for (int i = 0; i < n; i++) {
                AESCipher(state);
            }
